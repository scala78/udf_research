...
class UDFTest extends SparkFunSuite with LocalSparkContext {
  ...
  test("Extension add long literal to long literal") {
    ...
    checkAnswer(
      spark.sql("select c1, c2, addition_udf(c1,c2) from test_data"),
  ...
  test("Extension compute hash xx3") {
    ...
    checkAnswer(
      spark.sql(
        "select c1, c2, hash_xx3_udf(c1),hash_xx3_udf(c2) from test_data"
      ),
  ...
  test("Extension compute hash xx3 and adding long literals") {
    ...
    checkAnswer(
      spark.sql(
        "select c1, c2, hash_xx3_udf(addition_udf(c1,c2)) from test_data"
      ),
